(function () {
    ...
    const svg = d3.select("#selected-countries-education-visualization svg")
        .attr("width", width)
    ...
    svg.append("text")
        ...
        .text("Relative Change in Fertility Rates (1950-2023)")
        .attr("font-size", "18px")
        .attr("fill", "#333");
    ...
    // Load both datasets
    Promise.all([
        d3.csv("fertility_rate_relative_change.csv"),
        d3.csv("fertility_with_workweek_and_education_data.csv")
    ]).then(([relativeChangeData, educationData]) => {
        // Parse data
        relativeChangeData.forEach(d => {
        ...
        const highlightedCountries = educationData
            .filter(d => d["Fertility Rate"] < 1.5 && d["Years of Education"] > 13)
            .map(d => d["Entity"]); // Get country names

        // Define scales
        const fixedRadius = 7;
        const opacityScale = d3.scaleLinear()
            .domain([0, d3.max(relativeChangeData, d => Math.abs(d["Relative Change"]))])
            .range([0.2, 1]);
        ...
        ranges.forEach((range, i) => {
            ...
            const countriesInRange = relativeChangeData.filter(d => d["Relative Change"] <= range && d["Relative Change"] > nextRange);
            ...
            countriesInRange.forEach(country => {
                ...
                const circle = chart.append("circle")
                    ...
                    .attr("cy", position.y)
                    .attr("r", fixedRadius)
                    .attr("fill", "rgb(105, 201, 129)")
                    .attr("opacity", opacityScale(Math.abs(country["Relative Change"])))
                    .attr("stroke", highlightedCountries.includes(country["Country"]) ? "black" : "rgb(105, 201, 129)")
                    .attr("stroke-width", highlightedCountries.includes(country["Country"]) ? 3 : 1);
                ...
                // Add country label for highlighted circles
                if (highlightedCountries.includes(country["Country"])) {
                    chart.append("text")
